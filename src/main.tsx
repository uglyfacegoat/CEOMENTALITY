...
function App() {
 ...
 useEffect(()=>{const media=window.matchMedia('(max-width: 900px)');const update=()=>{if(!media.matches)setMobileSidebarOpen(false)};media.addEventListener('change',update);return()=>media.removeEventListener('change',update)},[]);
 ...
 const navigate=(t:AppRoute)=>runPageTransition(()=>{setActive(t);setSelected(null);writeRoute(t);setQuery('');setFilters({status:'All',wave:'All',source:'All',expertise:'All',codeType:'All',expiry:'All',drop:'All drops'})}); const mode=selected?'Candidate Dossiers':active;
 const openCandidate=(id:string)=>runPageTransition(()=>{setActive('Candidate Dossiers');setSelected(id);writeRoute('Candidate Dossiers',id)});
 const closeCandidate=()=>runPageTransition(()=>{setSelected(null);writeRoute('Candidate Dossiers')});
 const moveCandidate=(direction:number)=>{const index=state.candidates.findIndex(c=>c.id===selected);const next=(index+direction+state.candidates.length)%state.candidates.length;const id=state.candidates[next]?.id??null;runPageTransition(()=>{setSelected(id);writeRoute('Candidate Dossiers',id)})};
 const selectedCandidate=selected?state.candidates.find(c=>c.id===selected):undefined;
 const toggleSidebar=()=>{if(window.matchMedia('(max-width: 900px)').matches){setMobileSidebarOpen(value=>!value);return}setSidebarCollapsed(value=>{const next=!value;localStorage.setItem('ceomentality:sidebar',next?'collapsed':'open');return next})};
 return <><Header active={mode} navigate={navigate} candidates={state.candidates} openCandidate={openCandidate} openProfile={()=>navigate('Profile')} onLogout={logout} language={language} setLanguage={changeLanguage} hasSidebar={active!=='Profile'} desktopSidebarOpen={!sidebarCollapsed} mobileSidebarOpen={mobileSidebarOpen} toggleSidebar={toggleSidebar}/><div className={`app-shell ${active==='Profile'?'profile-shell':''} ${sidebarCollapsed?'sidebar-collapsed':''} ${mobileSidebarOpen?'mobile-sidebar-open':''}`}>{active!=='Profile'&&<><Sidebar mode={mode} query={query} setQuery={setQuery} filters={filters} setFilters={setFilters} candidates={state.candidates} onClose={()=>setMobileSidebarOpen(false)} reset={()=>{setQuery('');setFilters({status:'All',wave:'All',source:'All',expertise:'All',codeType:'All',expiry:'All',drop:'All drops'})}}/><span className="sidebar-reveal-zone"/><button className="sidebar-toggle" onClick={toggleSidebar} aria-label={sidebarCollapsed?'Show filters':'Hide filters'}><Icon name="sidebar" size={20}/></button><button className="mobile-sidebar-scrim" aria-label="Close filters" onClick={()=>setMobileSidebarOpen(false)}/></>}{active==='Profile'?<ProfilePage notify={notify} onBack={()=>navigate('Candidate Dossiers')}/>:selectedCandidate?<Dossier candidate={selectedCandidate} back={closeCandidate} update={updateCandidate} addNote={n=>{updateCandidate({...selectedCandidate,notes:[...selectedCandidate.notes,n]})}} notify={notify} onPrevious={()=>moveCandidate(-1)} onNext={()=>moveCandidate(1)}/>:active==='Candidate Dossiers'?<Dossiers candidates={state.candidates} open={openCandidate} query={query} filters={filters}/>:active==='Access Codes'?<Access drops={state.drops} query={query} filters={filters} notify={notify} createDrop={()=>setModal('drop')} generateBatch={generateBatch} resetFilters={()=>{setQuery('');setFilters({status:'All',wave:'All',source:'All',codeType:'All',expiry:'All',drop:'All drops'})}} closeDrop={id=>{setState({...state,drops:state.drops.map(d=>d.id===id?{...d,status:d.status==='Active'?'Closed':'Active'}:d)});notify('Drop status updated')}} exportData={exportData}/>:<Analytics candidates={state.candidates} drops={state.drops} filters={filters} exportData={exportData}/>}</div>{modal==='candidate'&&<CandidateForm onClose={()=>setModal(null)} onSubmit={addCandidate}/>} {modal==='drop'&&<DropForm onClose={()=>setModal(null)} onSubmit={addDrop}/>} {toast&&<div className="toast">{toast}</div>}</>;
}
