...
function App() {
 const [state,setState]=useState<WorkspaceState|null>(null),[active,setActive]=useState<AppRoute>('Candidate Dossiers'),[selected,setSelected]=useState<string|null>(null),[query,setQuery]=useState(()=>getSavedView()?.query||''),[filters,setFilters]=useState<WorkspaceFilters>(()=>getSavedView()?.filters||defaultFilters),[modal,setModal]=useState<'candidate'|'drop'|null>(null),[toast,setToast]=useState(''),[sidebarCollapsed,setSidebarCollapsed]=useState(()=>localStorage.getItem('ceomentality:sidebar')==='collapsed'),[authenticated,setAuthenticated]=useState(()=>localStorage.getItem('ceomentality:session')==='active'),[language,setLanguage]=useState<Language>(()=>localStorage.getItem('ceomentality:language')==='ru'?'ru':'en');
 useEffect(()=>{api.load().then(setState)},[]); useEffect(()=>{if(state)api.save(state)},[state]);
 useEffect(()=>{localStorage.setItem('ceomentality:language',language);document.documentElement.lang=language;const apply=()=>localizeDocument(document.body,language);apply();const observer=new MutationObserver(apply);observer.observe(document.body,{childList:true,subtree:true,characterData:true});return()=>observer.disconnect()},[language]);
 ...
 const navigate=(t:AppRoute)=>{setActive(t);setSelected(null);setQuery('');setFilters({status:'All',wave:'All',source:'All',expertise:'All',codeType:'All',expiry:'All',drop:'All drops'})}; const mode=selected?'Candidate Dossiers':active;
 const openCandidate=(id:string)=>{setActive('Candidate Dossiers');setSelected(id)};
 const moveCandidate=(direction:number)=>{const index=state.candidates.findIndex(c=>c.id===selected);const next=(index+direction+state.candidates.length)%state.candidates.length;setSelected(state.candidates[next]?.id??null)};
